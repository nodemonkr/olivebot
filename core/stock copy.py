...
def setup(bot):
    ...
    @bot.tree.command(name="주식배정", description="관리자가 여러 유저에게 주식을 배정합니다.")
    ...
    async def 주식배정(
        ...
        target_list = [u for u in [user1, user2, user3, user4, user5] if u]
        if not target_list:
            await interaction.response.send_message("👥 최소 한 명 이상의 유저를 선택해야 합니다.", ephemeral=True)
            return

        embed = discord.Embed(
            title=f"📦 {name} 주식 배정 결과",
        ...
        for u in target_list:
            uid = str(u.id)
            users.setdefault(uid, {"olive": 0, "stocks": {}})
            users[uid].setdefault("stocks", {})
            holding = users[uid]["stocks"].get(name, {"qty": 0, "avg": 0})

            holding["qty"] += qty
            holding["avg"] = stocks[name]["price"]
            users[uid]["stocks"][name] = holding

            member = interaction.guild.get_member(u.id) or await interaction.guild.fetch_member(u.id)
            display_name = member.display_name if member else u.name
            embed.add_field(name=f"👤 {display_name}", value=f"{qty}주 지급 완료", inline=False)
